	...
	do
			...
			for i = 1, #GamePasses do
				...
				if not ProductsScrollFrame:FindFirstChild(Gamepass.Name) then
					...
					ImageButton.MouseButton1Click:Connect(function()
						...
						DescriptionSection:FindFirstChild("Description").Text = Gamepass.Description
							or "No description, please report in #Bug-Report"
			...
			for i = 1, #Items do
				...
				if not ProductsScrollFrame:FindFirstChild(Item.Name) then
					...
					ImageButton.MouseButton1Click:Connect(function()
						...
						DescriptionSection:FindFirstChild("Description").Text = Item.Description
							or "No description, please report in #Bug-Report"
		...
		DescriptionSection.Purchase.MouseButton1Click:Connect(function()
			if string.match(DescriptionSection.Purchase.Text, "Robux") then
				MarketService:PurchaseProduct(
					DescriptionSection:GetAttribute("AssetId"),
					DescriptionSection:GetAttribute("InfoType")
				)
			end
		end)
	end
		...
		LobbyService.CurrentLobbies:Observe(function(CurrentLobbies)
			...
			coroutine.wrap(function()
				for i, GuiObject: GuiObject in JoinLobbyScroll:GetChildren() do
					if
						not table.find(CurrentLobbies, Players:GetUserIdFromNameAsync(GuiObject.Name))
						and GuiObject ~= SampleJoinLobby
			...
			for LobbyUserId, LobbyData: LobbyData in pairs(CurrentLobbies) do
				if not JoinLobbyScroll:FindFirstChild(LobbyData.LobbyOwner.Name) then
					...
					Clone.Visible = false
					Clone.PlayerIcon.Image = Players:GetUserThumbnailAsync(
						LobbyUserId,
				...
				if
					LobbyData.Visibility == "Friends"
					and LobbyData.LobbyOwner:IsFriendsWith(Player.UserId)
					and LobbyData.IsCreated
				then
					LobbyGui.Visible = true
				elseif
					LobbyData.Visibility == "Friends"
					and not LobbyData.LobbyOwner:IsFriendsWith(Player.UserId)
					and LobbyData.Visibility ~= "Public"
				then
				...
				if table.find(LobbyData.PlayersInLobby, Player) and LobbyData.IsCreated then
					...
					for _, OtherPlayerLobbyGui: Instance in ipairs(LobbyScroll:GetChildren()) do
						if
							not table.find(CurrentLobbies, LobbyUserId)
							and OtherPlayerLobbyGui ~= SampleOwnerPlayerGui
							and OtherPlayerLobbyGui ~= SampleInLobbyPlayerGui
							and not OtherPlayerLobbyGui:IsA("UIListLayout")
						then
						...
						if not LobbyScroll:FindFirstChild(OtherPlayer.Name) then
							if _IsOwner then
								if OtherPlayer == Player then
									...
									OwnerGui.PlayerIcon.Image = Players:GetUserThumbnailAsync(
										Player.UserId,
										Enum.ThumbnailType.HeadShot,
										Enum.ThumbnailSize.Size150x150
									)
									...
									OtherPlayerGui.PlayerIcon.Image = Players:GetUserThumbnailAsync(
										OtherPlayer.UserId,
										Enum.ThumbnailType.HeadShot,
										Enum.ThumbnailSize.Size150x150
									)
					...
					if BreakSignal then
						if ActivateSignal then
							...
							local tweenInfo = TweenInfo.new(
								BlackSwipe.Size.X.Offset / ViewportSize.X / 6,
