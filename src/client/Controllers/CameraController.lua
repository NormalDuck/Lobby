...
function CameraController:LockToPart(Part: Part, Yields: boolean, EasingStyle: Enum.EasingStyle, Time: number)
    ...
    if not Player:HasAppearanceLoaded() then
        task.spawn(function()
            ...
            local tweenInfo = TweenInfo.new((Time or Distance / self.CameraSpeed), (EasingStyle or Enum.EasingStyle.Linear), Enum.EasingDirection.InOut)
...
function CameraController:Unlock(EasingStyle: Enum.EasingStyle, Time: number)
    task.spawn(function()
        ...
        if EasingStyle then
            ...
            local tweenInfo = TweenInfo.new((Time or Distance / 100) , (EasingStyle or Enum.EasingStyle.Linear), Enum.EasingDirection.InOut)
            local Tween = TweenService:Create(CurrentCamera, tweenInfo, {CFrame = HumanoidRootPart.CFrame - Vector3.new(-10, 0, 0)})
            Tween:Play()
            Tween.Completed:Once(function()
