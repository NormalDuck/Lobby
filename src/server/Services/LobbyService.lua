...
function LobbyService.Client:EditLobby(Player: Player, Configurations: Configurations)
    ...
    do
        if MarketplaceService:UserOwnsGamePassAsync(Player.UserId, 690608475) then
            ...
            else print("Seed Failed Sanity Check") return end
        else
            if type(Configurations.Seed) ~= "nil" and not Configurations.Seed == 0 then print("Seed Failed Sanity Check") return end
        end
        ...
        if MarketplaceService:UserOwnsGamePassAsync(Player.UserId, 690931056) then
            if type(Configurations.LobbySize) == "number" or type(Configurations.LobbySize) == "nil" then
                if not NumberInRange(Configurations.LobbySize, 1, 8) then return end
            else print("Lobby Size Failed Sanity Check") return end
        else
            if type(Configurations.LobbySize) == "number" or type(Configurations.LobbySize) == "nil" then
                if not NumberInRange(Configurations.LobbySize, 1, 4) then print("Lobby Size Failed Sanity Check") return end
            else print("Lobby Size Failed Sanity Check") return end
        end

        if not NumberInRange(Configurations.MazeSize, 10, 150) then return print("Maze Size Failed") end
        if Configurations.Visibility ~= "Public" and Configurations.Visibility ~= "Private" and Configurations.Visibility ~= "Friends" then return print("Invaild statement") end
        if RequestedLobby.IsCreated then return print("Requested Lobby Cannot edit due to it createed") end
    end
    --[[Sanity Checks]]--
    if MarketplaceService:UserOwnsGamePassAsync(Player.UserId, 690608475) then
...
function LobbyService.Client:LeaveLobby(Player: Player, LobbyOwner: Player): boolean
    local RequestedLobby = self.Server.CurrentLobbies[LobbyOwner.UserId]
    if RequestedLobby.LobbyOwner == Player then return end
