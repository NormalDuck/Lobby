...
function DataService:KnitStart()
	...
	MarketplaceService.ProcessReceipt = function(ReceiptInfo)
		...
		elseif InfoType == Enum.InfoType.GamePass then
			...
			print(InfoType)
			warn("[DataService] " .. " invaild infotype for " .. ReceiptInfo.ProductId .. " ProductId")
		end
...
function DataService:KnitInit()
	Players.PlayerAdded:Connect(function(player)
		if RunService:IsStudio() then
			return warn("Cannot run DataService under studio - Currently paused")
		end
		local DataStore = DataStoreModule.new("Player", player.UserId)
		while DataStore.State == false do
			if DataStore:Open(Templete) ~= "Success" then
				task.wait(6)
			end
